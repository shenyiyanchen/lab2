...
default_alloc_pages(size_t n) {
    ...
    while ((le = list_next(le)) != &free_list) {	//遍历整个空闲链表
        ...
        if(p->property >= n){						//找到合适的空闲页
            if(p->property>n){						//判断页块大小大于所需到校
                struct Page *np = p+n;				//转换页结构
                np->property = p->property - n;		//分割页块
                SetPageProperty(np);				//设置标志位
                list_add_after(&(p->page_link),&(np->page_link));		//加入空闲链表
            }
            SetPageReserved(p);
            ClearPageProperty(p);
            set_page_ref(p,0);						//清除引用此页的虚拟页的个数
            list_del(&(p->page_link));				//清楚空闲链表中的页
...
default_free_pages(struct Page *base, size_t n) {
    ...
    while (le != &free_list) {
        ...
        if (base + n == p) {		                //如果是高位，则向高地址合并
            base->property += p->property;
            ClearPageProperty(p);
            list_del(&(p->page_link));
        }
        else if (p + p->property == base) {		    //如果是低位且在范围内，则向低地址合并
